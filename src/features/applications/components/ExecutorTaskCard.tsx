...
import type { ExecutorTaskItem } from '../types/application.types';

interface ExecutorTaskCardProps {
  task: ExecutorTaskItem;
}

/**
 * Card component for tasks the executor has applied to
 */
export function ExecutorTaskCard({ task }: ExecutorTaskCardProps) {
  return (
      ...
      <div className="overflow-hidden rounded-xl border bg-card shadow-card transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-card-hover">
        ...
        <div className="p-4">
          <div className="mb-1 flex items-start justify-between gap-2">
            ...
            <ApplicationStatusBadge statusName={task.applicationStatusName} />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 border-t pt-3 text-xs text-muted-foreground">
            ...
            {!task.image && task.isArchived && (
              <Badge variant="outline" className="ml-auto bg-gray-50 text-gray-500 border-gray-200">
                <Archive className="mr-1 h-3 w-3" />
                Archived
              </Badge>
            )}
